...
// $Source: /usr/local/cvs/OpenSees/SRC/element/forceBeamColumn/Truss2D.cpp,v $
...
#include <Parameter.h>
#include </home/chenting/Chenting/OpenSees/DEVELOPER/element/cpp/Truss2D.h>
#include </home/chenting/Chenting/OpenSees/SRC/element/nonlinearBeamColumn/matrixutil/MatrixUtil.h>
#include <Domain.h>
#include <Channel.h>
...
#include </home/chenting/Chenting/OpenSees/SRC/recorder/response/CompositeResponse.h>
...
Matrix Truss2D::theMatrix(12,12);
Vector Truss2D::theVector(12);
double Truss2D::workArea[200];

Vector *Truss2D::vsSubdivide = 0;
Matrix *Truss2D::fsSubdivide = 0;
Vector *Truss2D::SsrSubdivide = 0;
...
void* OPS_Truss2D()
{
    ...
    Element *theEle =  new Truss2D(iData[0],iData[1],iData[2],secTags.Size(),sections,
...
Truss2D::Truss2D():
  Element(0,741), connectedExternalNodes(2),
  ...
  fs(0), vs(0), Ssr(0), vscommit(0), sp(0), Ki(0), isTorsion(false)
{
  ...
  if (!vsSubdivide || !fsSubdivide || !SsrSubdivide) {
    opserr << "Truss2D::Truss2D() -- failed to allocate Subdivide arrays";
    exit(-1);
  }
}
  ...
  fs(0), vs(0),Ssr(0), vscommit(0), sp(0), Ki(0), isTorsion(false)
{
  ...
  if (beamIntegr == 0) {
    opserr << "Error: Truss2D::Truss2D: could not create copy of beam integration object" << endln;
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "Error: Truss2D::Truss2D: could not create copy of coordinate transformation object" << endln;
    exit(-1);
  }
...
Truss2D::setDomain(Domain *theDomain)
{
  ...
  if (theDomain == 0) {
    ...
    opserr << "Truss2D::setDomain:  theDomain = 0 ";
    exit(0); 
  }
  ...
  if (theNodes[0] == 0) {
    opserr << "Truss2D::setDomain: Nd1: ";
    opserr << Nd1 << "does not exist in model\n";
    exit(0);
  }
  
  if (theNodes[1] == 0) {
    opserr << "Truss2D::setDomain: Nd2: ";
  ...
  if ((dofNode1 != NND) || (dofNode2 != NND)) {
    opserr << "Truss2D::setDomain(): Nd2 or Nd1 incorrect dof ";
    exit(0);
  }
   
  // initialize the transformation
  if (crdTransf->initialize(theNodes[0], theNodes[1])) {
    opserr << "Truss2D::setDomain(): Error initializing coordinate transformation";
    exit(0);
  }
  ...
  double L = 0.032;
  if (L == 0.0) {
    opserr << "Truss2D::setDomain(): Zero element length:" << this->getTag();
    exit(0);
  }
...
Truss2D::commitState()
{
  ...
  if ((err = this->Element::commitState()) != 0) {
    opserr << "Truss2D::commitState () - failed in base class";
  }    
...
int Truss2D::revertToLastCommit()
{
  int err;
  int i = 0;
...
Truss2D::getInitialStiff(void)
{
  ...
  if (f.Solve(I, kvInit) < 0)
    opserr << "Truss2D::getInitialStiff() -- could not invert flexibility";
  ...
  const Vector &
  Truss2D::getResistingForce(void)
  {
  ...
  void
  Truss2D::initializeSectionHistoryVariables (void)
  {
    for (int i = 0; i < numSections; i++) {
  ...
  Truss2D::update()
  {
	...
	if (initialFlag != 2) {
	  ...
	  for (j=0; j <numIters; j++) {
		...
		opserr << "Truss2D::update() - section failed in setTrial\n";
		return -1;
	      }
	    ...
	    if (f.Solve(I, kvTrial) < 0)
	      opserr << "Truss2D::update() -- could not invert flexibility\n";
    ...
    if (converged == false) {
      opserr << "WARNING - Truss2D::update - failed to get compatible ";
  ...
  void Truss2D::getForceInterpolatMatrix(double xi, Matrix &b, const ID &code)
  {
    b.Zero();
  ...
  void Truss2D::getDistrLoadInterpolatMatrix(double xi, Matrix &bp, const ID &code)
  {
    bp.Zero();
  ...
  int
  Truss2D::addLoad(ElementalLoad *theLoad, double loadFactor)
  {
    int type;
    ...
    if (sp == 0) {
      ...
      if (sp == 0) {
	opserr << "Truss2D::addLoad -- out of memory\n";
	exit(-1);
      }
    }
	...
	if (x <= a) {
      ...
      opserr << "Truss2D::addLoad() -- load type unknown for element with tag: " <<
	this->getTag() << endln;
  ...
  int 
  Truss2D::addInertiaLoadToUnbalance(const Vector &accel)
  {
  ...
  const Vector &
  Truss2D::getResistingForceIncInertia()
  {	
  ...
  {  
    ...
    if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
      opserr << "Truss2D::sendSelf() - failed to send ID data\n";
      return -1;
    }    
    ...
    if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
      opserr << "Truss2D::sendSelf() - failed to send crdTranf\n";
      return -1;
    }      

    if (beamIntegr->sendSelf(commitTag, theChannel) < 0) {
      opserr << "Truss2D::sendSelf() - failed to send beamIntegr\n";
      return -1;
    }      
	...
	opserr << "Truss2D::sendSelf() - section " <<
    ...
    if (theChannel.sendVector(dbTag, commitTag, dData) < 0) {
       opserr << "Truss2D::sendSelf() - failed to send Vector data\n";
  ...
  Truss2D::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
  {
    ...
    if (theChannel.recvID(dbTag, commitTag, idData) < 0)  {
      opserr << "Truss2D::recvSelf() - failed to recv ID data\n";
	...
	if (crdTransf == 0) {
	    opserr << "Truss2D::recvSelf() - failed to obtain a CrdTrans object with classTag" <<
    ...
    {
       opserr << "Truss2D::sendSelf() - failed to recv crdTranf\n";
       return -3;
    }      
	...
	if (beamIntegr == 0) {opserr << "Truss2D::recvSelf() - failed to obtain the beam integration object with classTag" <<
    ...
    {
       opserr << "Truss2D::sendSelf() - failed to recv beam integration\n";
	...
	delete [] vscommit;
      ...
      if (vscommit == 0) {
	opserr << "Truss2D::recvSelf -- failed to allocate vscommit array\n";
	return -1;
      }
	...
	delete [] fs;
      ...
      if (fs == 0) {
	opserr << "Truss2D::recvSelf -- failed to allocate fs array\n";
	return -1;
      }
	...
	delete [] vs;
      ...
      if (vs == 0) {
	opserr << "Truss2D::recvSelf -- failed to allocate vs array\n";
	return -1;
      }
	...
	delete [] Ssr;
      ...
      if (Ssr == 0) {
	opserr << "Truss2D::recvSelf -- failed to allocate Ssr array\n";
	...
	return -1;
      ...
      if (sections == 0) {
	opserr << "Truss2D::recvSelf() - out of memory creating sections array of size" <<
	  idData(3) << endln;
	exit(-1);
      }    
	...
	if (sections[i] == 0) {
	  opserr << "Truss2D::recvSelf() - Broker could not create Section of class type" <<
	    sectClassTag << endln;
	  exit(-1);
	}
	...
	if (sections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
	  opserr << "Truss2D::recvSelf() - section " <<
    ...
    if (theChannel.recvVector(dbTag, commitTag, dData) < 0)  {
      opserr << "Truss2D::recvSelf() - failed to send Vector data\n";
      return -1;
    }    
  ...
  int
  Truss2D::getInitialFlexibility(Matrix &fe)
  {
    fe.Zero();
...
int
Truss2D::getInitialDeformations(Vector &v0)
{
  v0.Zero();
  ...
  void
  Truss2D::compSectionDisplacements(Vector sectionCoords[],
					      Vector sectionDispls[]) const
  {
	 ...
	 if (sectionKey1 == 0) {
	   opserr << "FATAL Truss2D::compSectionResponse - section does not provide Mz response\n";
	   exit(-1);
	 }
	 if (sectionKey2 == 0) {
	   opserr << "FATAL Truss2D::compSectionResponse - section does not provide My response\n";
	   exit(-1);
	 }
  ...
  Truss2D::Print(OPS_Stream &s, int flag)
  {
    ...
    // flag set to 2 used to print everything .. used for viewing data for UCSD renderer  
	 ...
	 if (!coords) {
	   opserr << "Truss2D::Print() -- failed to allocate coords array";
	   exit(-1);
	 }
	 ...
	 sections[i]->Print(s, flag); 
       ...
       s << "\nElement: " << this->getTag() << " Type: Truss2D ";
  ...
  OPS_Stream &operator<<(OPS_Stream &s, Truss2D &E)
  {
    E.Print(s);
    return s;
  }
  ...
  int
  Truss2D::displaySelf(Renderer &theViewer, int displayMode, float fact, const char **displayModes, int numModes)
  {
...
int
Truss2D::setParameter(const char **argv, int argc, Parameter &param)
{
  if (argc < 1)
    return -1;
...
int
Truss2D::updateParameter (int parameterID, Information &info)
{
  ...
  if (parameterID == 1) {
    ...
    if (ok < 0) {
      opserr << "Truss2D::updateParameter() - could not update parameter. " << endln;
      return ok;
    }
    else {
      return ok;
...
void
Truss2D::setSectionPointers(int numSec, SectionForceDeformation **secPtrs)
{
  if (numSec > maxNumSections) {
    opserr << "Error: Truss2D::setSectionPointers -- max number of sections exceeded";
  }
  
  numSections = numSec;
  
  if (secPtrs == 0) {
    opserr << "Error: Truss2D::setSectionPointers -- invalid section pointer";
  }	  
  
  sections = new SectionForceDeformation *[numSections];
  if (sections == 0) {
    opserr << "Error: Truss2D::setSectionPointers -- could not allocate section pointers";
  }  
  
  for (int i = 0; i < numSections; i++) {
    
    if (secPtrs[i] == 0) {
      opserr << "Error: Truss2D::setSectionPointers -- null section pointer " << i << endln;
    }
    
    sections[i] = secPtrs[i]->getCopy();
    
    if (sections[i] == 0) {
      opserr << "Error: Truss2D::setSectionPointers -- could not create copy of section " << i << endln;
    }
  ...
  if (!isTorsion)
    opserr << "Truss2D::Truss2D -- no torsion detected in sections, " <<
  ...
  fs  = new Matrix [numSections];
  if (fs == 0) {
    opserr << "Truss2D::setSectionPointers -- failed to allocate fs array";
  }
  
  vs = new Vector [numSections];
  if (vs == 0) {
    opserr << "Truss2D::setSectionPointers -- failed to allocate vs array";
  }
  
  Ssr  = new Vector [numSections];
  if (Ssr == 0) {
    opserr << "Truss2D::setSectionPointers -- failed to allocate Ssr array";
  }
  
  vscommit = new Vector [numSections];
  if (vscommit == 0) {
    opserr << "Truss2D::setSectionPointers -- failed to allocate vscommit array";
  }
