...
//new SectionAggregator (tag, *theSec, nMats, &theMats[0], codes);
//SectionAggregator (tag, nMats, &theMats[0], codes);
...
SectionAggregator::setParameter(const char **argv, int argc, Parameter &param)
{
  ...
  // Check if the parameter belongs to only the section
    ...
    opserr << "SectionAggregatorIs Used " << endln;
    if (argc < 2) {
    ...
    if (theSection != 0) {
    opserr << "SectionAggregatorIs Used 2" << endln;
