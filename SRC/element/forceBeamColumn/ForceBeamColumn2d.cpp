...
void* OPS_ForceBeamColumn2d()
{
	...
	} else if(strcmp(type,"-lenfactor") == 0) {
	    if(OPS_GetNumRemainingInputArgs() > 0) {
		if(OPS_GetDoubleInput(&numData,&lenfactor) < 0) {
		    opserr << "WARNING invalid lenfactor\n";
		    return 0;
		}
	    }
	}
...
int OPS_ForceBeamColumn2d(Domain& theDomain, const ID& elenodes, ID& eletags)
{
	...
	if(strcmp(type,"-iter") == 0) {
	    ...
	    if(OPS_GetNumRemainingInputArgs() > 0) {
		if(OPS_GetDoubleInput(&numData,&lenfactor) < 0) return -1;
	    }
	}
    }
	...
	theEle = new ForceBeamColumn2d(--currTag,elenodes(2*i),elenodes(2*i+1),secTags.Size(),
				       sections,*bi,*theTransf,mass,maxIter,tol,lenfactor);
	if (theEle == 0) {
...
ForceBeamColumn2d::ForceBeamColumn2d(): 
  ...
  rho(0.0), maxIters(0), tol(0.0),lenfactor(0.0),
  initialFlag(0),
...
ForceBeamColumn2d::ForceBeamColumn2d (int tag, int nodeI, int nodeJ,
				      ...
				      int maxNumIters, double tolerance, double LenFactor):
  Element(tag,ELE_TAG_ForceBeamColumn2d), connectedExternalNodes(2),
  beamIntegr(0), numSections(0), sections(0), crdTransf(0),
  rho(massDensPerUnitLength),maxIters(maxNumIters), tol(tolerance), lenfactor(LenFactor),
  initialFlag(0),
  kv(NEBD,NEBD), Se(NEBD), 
  kvcommit(NEBD,NEBD), Secommit(NEBD),
  fs(0), vs(0),Ssr(0), vscommit(0), 
...
ForceBeamColumn2d::getResponse(int responseID, Information &eleInfo)
{
  ...
  if (responseID == 1)
    ...
    theVector(2) = Se(1);
    theVector(5) = Se(2);
    double V = (Se(1)+Se(2))/(crdTransf->getInitialLength() * lenfactor);
    theVector(1) =  V+p0[1];
    theVector(4) = -V+p0[2];
