...
// $Source: /usr/local/cvs/OpenSees/SRC/element/Bolt/Bolt3d.h,v $
...
class Bolt: public Element
{
 public:
  Bolt();
  Bolt(int tag, int nodeI, int nodeJ,
  ...
  ~Bolt();
  const char *getClassType(void) const {return "Bolt";};
  ...
  friend OPS_Stream &operator<<(OPS_Stream &s, Bolt &E);
