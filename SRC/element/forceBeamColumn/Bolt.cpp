...
// $Source: /usr/local/cvs/OpenSees/SRC/element/forceBeamColumn/Bolt.cpp,v $
...
Matrix Bolt::theMatrix(12,12);
Vector Bolt::theVector(12);
double Bolt::workArea[200];

Vector *Bolt::vsSubdivide = 0;
Matrix *Bolt::fsSubdivide = 0;
Vector *Bolt::SsrSubdivide = 0;
...
void* OPS_Bolt()
{
    ...
    Element *theEle =  new Bolt(iData[0],iData[1],iData[2],secTags.Size(),sections,
...
Bolt::Bolt():
  Element(0,ELE_TAG_Bolt), connectedExternalNodes(2),
  ...
  fs(0), vs(0), Ssr(0), vscommit(0), sp(0), Ki(0), isTorsion(false)
{
  ...
  if (!vsSubdivide || !fsSubdivide || !SsrSubdivide) {
    opserr << "Bolt::Bolt() -- failed to allocate Subdivide arrays";
    exit(-1);
  }
}
  ...
  fs(0), vs(0),Ssr(0), vscommit(0), sp(0), Ki(0), isTorsion(false)
{
  ...
  if (beamIntegr == 0) {
    opserr << "Error: Bolt::Bolt: could not create copy of beam integration object" << endln;
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "Error: Bolt::Bolt: could not create copy of coordinate transformation object" << endln;
    exit(-1);
  }
...
int
Bolt::getNumExternalNodes(void) const
{
  return 2;
}

const ID &
Bolt::getExternalNodes(void)
{
...
void
Bolt::setDomain(Domain *theDomain)
{
  ...
  if (theDomain == 0) {
    ...
    opserr << "Bolt::setDomain:  theDomain = 0 ";
    exit(0); 
  }
  ...
  if (theNodes[0] == 0) {
    opserr << "Bolt::setDomain: Nd1: ";
    opserr << Nd1 << "does not exist in model\n";
    exit(0);
  }
  
  if (theNodes[1] == 0) {
    opserr << "Bolt::setDomain: Nd2: ";
  ...
  if ((dofNode1 != NND) || (dofNode2 != NND)) {
    opserr << "Bolt::setDomain(): Nd2 or Nd1 incorrect dof ";
    exit(0);
  }
   
  // initialize the transformation
  if (crdTransf->initialize(theNodes[0], theNodes[1])) {
    opserr << "Bolt::setDomain(): Error initializing coordinate transformation";
    exit(0);
  }
  ...
  double L = 0.032;
  if (L == 0.0) {
    opserr << "Bolt::setDomain(): Zero element length:" << this->getTag();
    exit(0);
  }
...
Bolt::commitState()
{
  ...
  if ((err = this->Element::commitState()) != 0) {
    opserr << "Bolt::commitState () - failed in base class";
  }    
...
Bolt::getInitialStiff(void)
{
  ...
  if (f.Solve(I, kvInit) < 0)
    opserr << "Bolt::getInitialStiff() -- could not invert flexibility";
  ...
  void
  Bolt::initializeSectionHistoryVariables (void)
  {
    for (int i = 0; i < numSections; i++) {
  ...
  Bolt::update()
  {
	...
	if (initialFlag != 2) {
	  ...
	  for (j=0; j <numIters; j++) {
		...
		opserr << "Bolt::update() - section failed in setTrial\n";
		return -1;
	      }
	    ...
	    if (f.Solve(I, kvTrial) < 0)
	      opserr << "Bolt::update() -- could not invert flexibility\n";
    ...
    if (converged == false) {
      opserr << "WARNING - Bolt::update - failed to get compatible ";
  ...
  void Bolt::getForceInterpolatMatrix(double xi, Matrix &b, const ID &code)
  {
    b.Zero();
  ...
  void Bolt::getDistrLoadInterpolatMatrix(double xi, Matrix &bp, const ID &code)
  {
    bp.Zero();
  ...
  int
  Bolt::addLoad(ElementalLoad *theLoad, double loadFactor)
  {
    int type;
    ...
    if (sp == 0) {
      ...
      if (sp == 0) {
	opserr << "Bolt::addLoad -- out of memory\n";
	exit(-1);
      }
    }
	...
	if (x <= a) {
      ...
      opserr << "Bolt::addLoad() -- load type unknown for element with tag: " <<
	this->getTag() << endln;
  ...
  int 
  Bolt::addInertiaLoadToUnbalance(const Vector &accel)
  {
  ...
  const Vector &
  Bolt::getResistingForceIncInertia()
  {	
  ...
  int
  Bolt::sendSelf(int commitTag, Channel &theChannel)
  {  
    ...
    if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
      opserr << "Bolt::sendSelf() - failed to send ID data\n";
      return -1;
    }    
    ...
    if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
      opserr << "Bolt::sendSelf() - failed to send crdTranf\n";
      return -1;
    }      

    if (beamIntegr->sendSelf(commitTag, theChannel) < 0) {
      opserr << "Bolt::sendSelf() - failed to send beamIntegr\n";
      return -1;
    }      
    ...
    if (theChannel.sendVector(dbTag, commitTag, dData) < 0) {
       opserr << "Bolt::sendSelf() - failed to send Vector data\n";
  ...
  int
  Bolt::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
  {
    //
    ...
    if (theChannel.recvID(dbTag, commitTag, idData) < 0)  {
      opserr << "Bolt::recvSelf() - failed to recv ID data\n";
	...
	if (crdTransf == 0) {
	    opserr << "Bolt::recvSelf() - failed to obtain a CrdTrans object with classTag" <<
    ...
    {
       opserr << "Bolt::sendSelf() - failed to recv crdTranf\n";
       return -3;
    }      
	...
	if (beamIntegr == 0) {opserr << "Bolt::recvSelf() - failed to obtain the beam integration object with classTag" <<
    ...
    {
       opserr << "Bolt::sendSelf() - failed to recv beam integration\n";
	...
	delete [] vscommit;
      ...
      if (vscommit == 0) {
	opserr << "Bolt::recvSelf -- failed to allocate vscommit array\n";
	return -1;
      }
	...
	delete [] fs;
      ...
      if (fs == 0) {
	opserr << "Bolt::recvSelf -- failed to allocate fs array\n";
	return -1;
      }
	...
	delete [] vs;
      ...
      if (vs == 0) {
	opserr << "Bolt::recvSelf -- failed to allocate vs array\n";
	return -1;
      }
	...
	delete [] Ssr;
      ...
      if (Ssr == 0) {
	opserr << "Bolt::recvSelf -- failed to allocate Ssr array\n";
	...
	return -1;
      ...
      if (sections == 0) {
	opserr << "Bolt::recvSelf() - out of memory creating sections array of size" <<
	  idData(3) << endln;
	exit(-1);
      }    
	...
	if (sections[i] == 0) {
	  opserr << "Bolt::recvSelf() - Broker could not create Section of class type" <<
	    sectClassTag << endln;
	  exit(-1);
	}
    ...
    if (theChannel.recvVector(dbTag, commitTag, dData) < 0)  {
      opserr << "Bolt::recvSelf() - failed to send Vector data\n";
      return -1;
    }    
  ...
  int
  Bolt::getInitialFlexibility(Matrix &fe)
  {
    fe.Zero();
...
int
Bolt::getInitialDeformations(Vector &v0)
{
  v0.Zero();
  ...
  void
  Bolt::compSectionDisplacements(Vector sectionCoords[],
					      Vector sectionDispls[]) const
  {
	 ...
	 if (sectionKey1 == 0) {
	   opserr << "FATAL Bolt::compSectionResponse - section does not provide Mz response\n";
	   exit(-1);
	 }
	 if (sectionKey2 == 0) {
	   opserr << "FATAL Bolt::compSectionResponse - section does not provide My response\n";
	   exit(-1);
	 }
  ...
  void
  Bolt::Print(OPS_Stream &s, int flag)
  {
    ...
    // flag set to 2 used to print everything .. used for viewing data for UCSD renderer  
	 ...
	 if (!coords) {
	   opserr << "Bolt::Print() -- failed to allocate coords array";
	   exit(-1);
	 }
  ...
  OPS_Stream &operator<<(OPS_Stream &s, Bolt &E)
  {
    E.Print(s);
    return s;
  }
  ...
  int
  Bolt::displaySelf(Renderer &theViewer, int displayMode, float fact, const char **displayModes, int numModes)
  {
  ...
  Response*
  Bolt::setResponse(const char **argv, int argc, OPS_Stream &output)
  {
    Response *theResponse = 0;
    
    output.tag("ElementOutput");
    output.attr("eleType","Bolt");
...
int 
Bolt::getResponse(int responseID, Information &eleInfo)
{
  static Vector vp(6);
  static Matrix fe(6,6);
...
int
Bolt::setParameter(const char **argv, int argc, Parameter &param)
{
  if (argc < 1)
    return -1;
...
int
Bolt::updateParameter (int parameterID, Information &info)
{
  ...
  if (parameterID == 1) {
    ...
    if (ok < 0) {
      opserr << "Bolt::updateParameter() - could not update parameter. " << endln;
      return ok;
    }
    else {
      return ok;
...
void
Bolt::setSectionPointers(int numSec, SectionForceDeformation **secPtrs)
{
  if (numSec > maxNumSections) {
    opserr << "Error: Bolt::setSectionPointers -- max number of sections exceeded";
  }
  
  numSections = numSec;
  
  if (secPtrs == 0) {
    opserr << "Error: Bolt::setSectionPointers -- invalid section pointer";
  }	  
  
  sections = new SectionForceDeformation *[numSections];
  if (sections == 0) {
    opserr << "Error: Bolt::setSectionPointers -- could not allocate section pointers";
  }  
  
  for (int i = 0; i < numSections; i++) {
    
    if (secPtrs[i] == 0) {
      opserr << "Error: Bolt::setSectionPointers -- null section pointer " << i << endln;
    }
    
    sections[i] = secPtrs[i]->getCopy();
    
    if (sections[i] == 0) {
      opserr << "Error: Bolt::setSectionPointers -- could not create copy of section " << i << endln;
    }
  ...
  if (!isTorsion)
    opserr << "Bolt::Bolt -- no torsion detected in sections, " <<
  ...
  fs  = new Matrix [numSections];
  if (fs == 0) {
    opserr << "Bolt::setSectionPointers -- failed to allocate fs array";
  }
  
  vs = new Vector [numSections];
  if (vs == 0) {
    opserr << "Bolt::setSectionPointers -- failed to allocate vs array";
  }
  
  Ssr  = new Vector [numSections];
  if (Ssr == 0) {
    opserr << "Bolt::setSectionPointers -- failed to allocate Ssr array";
  }
  
  vscommit = new Vector [numSections];
  if (vscommit == 0) {
    opserr << "Bolt::setSectionPointers -- failed to allocate vscommit array";
  }
