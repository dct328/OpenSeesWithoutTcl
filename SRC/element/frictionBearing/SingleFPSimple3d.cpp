...
int SingleFPSimple3d::update()
{
    ...
    do  {
        ...
        //double N = -qb(0) + qb(1)/Ry*ub(1) + qb(2)/Rz*ub(2)- qb(1)*ul(5) + qb(2)*ul(4);
        double N = -qb(0);
        ...
        if (Y <= 0.0)  {
            ...
            qb(1) = qTrial(0) + k2y*ub(1);
            qb(2) = qTrial(1) + k2z*ub(2);
            // set tangent stiffnesses
            kb(1,1) = kb(2,2) = kInit;
            kb(1,2) = kb(2,1) = 0.0;
        }
        // plastic step -> return mapping
        else if(Y > 0 && ub(1) <= 2 && ub(2) <= 2) {
            ...
            qb(1) = qYield*qTrial(0)/qTrialNorm + k2y*ub(1);
            qb(2) = qYield*qTrial(1)/qTrialNorm + k2z*ub(2);
            ...
            if (sqrt(pow(qb(1)-qbOld(0),2)+pow(qb(2)-qbOld(1),2)) >= 1e14) {
                opserr << "The Shear Force is too large!!" << "\n";
                iter = maxIter + 1;
                break;
            ...
            //qb(2) = qYield*qTrial(1)/qTrialNorm + k2z*ub(2) + N*ul(4);
            if (ub(1) - 2 > 0) {
               qb(1) = qYield*qTrial(0)/qTrialNorm + k2y*ub(1) + 0.5*k2y*(ub(1) - 2);
               kb(1,1) =  qYield*k0y*qTrial(1)*qTrial(1)/D + 1.5*k2y;
            }
            else {
               qb(1) = qYield*qTrial(0)/qTrialNorm + k2y*ub(1);
               kb(1,1) =  qYield*k0y*qTrial(1)*qTrial(1)/D;
            }
            if (ub(2) - 2 > 0) {
               qb(2) = qYield*qTrial(1)/qTrialNorm + k2z*ub(2) + 0.5*k2z*(ub(2) - 2);
               kb(2,2) =  qYield*k0z*qTrial(0)*qTrial(0)/D + 1.5*k2z;
            }
            else {
               qb(2) = qYield*qTrial(1)/qTrialNorm + k2z*ub(2);
               kb(2,2) =  qYield*k0z*qTrial(0)*qTrial(0)/D;
            }
            // set tangent stiffnesses
    ...
    theMaterials[1]->setTrialStrain(0, 0);
    ...
    theMaterials[2]->setTrialStrain(0, 0);
    ...
    theMaterials[3]->setTrialStrain(0, 0);
    ...
    if (iter >= maxIter)   {
        if (sqrt(pow(qb(1)-qbOld(0),2)+pow(qb(2)-qbOld(1),2)) >= 1e-8) {
        opserr << "WARNING: SingleFPSimple3d::update() - element: "
